...
```RPS = DAU * (0.05 + 30 + 60 + 10 + 0.6 + 4) / 86400 = 105 * DAU / 86400 ~ 5600```
...
```azure
Трафик в секунду = (2.7 + 54 + 107 + 32 + 216) * 5KiB + 320 * 67MiB ~ 21 Gbit/sec 
```
...
```azure
Создание сслыки/изменение доступа = 32 * 5KiB ~ 160 KiB/sec
```

```azure
Просмотр папки = 4 * 5KiB ~ 20 KiB/sec
```

Скачивание будет производится с s3 так что посчитаем его отдельно.

```
Скачивание в секунду = 640 * 67MiB ~ 42 Gbit/sec 
```
...
| Трафик в секунду            | 60 Gbit/s  | 72 Gbit/s          | 12 Gbit/s      |
| Пиковый трафик с секунду    | 150 Gbit/s | 180 Gbit/s         | 30 Gbit/s      |
| RPS                         | 5600       | 6720               | 1120           |
...
можно оставить один датацентр, но на случай его поломки и чтобы точно выдержать нагрузку от пользователей поставим еще один датацентр в следущем по количеству пользователей регионе - **Красноддарский край**
