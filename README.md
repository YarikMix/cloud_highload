...
| files            | Таблица files хранит данные о файлах. Поле owner хранит FK на uuid пользователя, загрузившего файл. Файл может быть загружен как в корень облака, так и в папку, поэтому в поле parent хранится FK Nullable uuid родительской папки. Файлы могут быть пошерены, за это отвечает поле shared. Кроме этого файл содержит название (name), дату загрузки (created), а также ссылку на S3 хранилище (s3_url) |
| folders          | Таблица folders хранит данные о папках. // TODO                                                                                                                                                                                                                                                                                                                                                          |
| acl              | Таблица acl хранит информацию о пошаренных файлах / папках. // TODO                                                                                                                                                                                                                                                                                                                                      |
...
|------------------| ---------------- | ------------------------------------------------------------ | ------------- |
| users            | 3 500 000        | 16 + 40 + 40 + 40 + 256 = 392 б                              | 1,28 Гб       |
| files            | 12 000 000       | 16 + 16 + 16 + 256 + 50 + 1 + 4 + 4 = 363 б                  | 4,06 Гб       |
| folders          | 2 000 000        | 16 + 16 + 32 + 16 + 256 + 532\*5 + 256 + 4 + 4 + 16 = 3276 б | 6,1 Гб        |
| acl              | 40 000 000       | 16 + 16 + 32 + 16 + 50 + 256 + 4 = 390 б                     | 14,53 Гб      |
