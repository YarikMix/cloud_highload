...
* Перенесем поле permissions из shared в folder
* Добавим таблице folder поле origin_id чтобы хранить там id папки пошеренной папки

#### Как будет работать шаринг

Пользовать №1 (user1) шарит папку на пользователя №2 (user2)

Пользовать №1 указывает email пользоваетля №2 и выбирает тип доступа к папке (Только чтение / Запись)

Далее пользователь №2 может принять или отклонить запрос на шаринг папки

В случае, если пользователь №2 принимает запрос - пошаренная папка становится монтированной для пользователя №2
* Она отображается в списке папок на домашней странице
* Все файлы в ней синхронизируются с пользователем №1

На все вложенные файлы и папки распространяется доступ монтированной папки:
* Монтированная папка readonly -> все содержимое readonly
* Монтированная папка writable -> все содержимое writable

После монтирования папки - в таблицах folder и file создадутся копии содержимого корневой папки с правами на доступ

Примеры

user1 владеет папкой folder с  следующим содержимым:
- file1
- subfolder_1
    - file2
- subfolder_2
...
user1 шарит папку folder на user2 с доступом на чтение

user2 примонтирует папку folder и затем в бд появятся 7 новых записей:
* 3 записи в таблице files (file1, file2, file3)
  * owner: user2
* Запись folder в folders (*)
  * origin_id: uuid папки
  * owner: user2
  * permissions: readonly
* 3 записи в таблице folders (subfolder_1, subfolder_2, subfolder_3)
  * owner: user2
  * permissions: readonly

#### Сценирий №2

При открытии файлов в монтированной папке проверяется доступ родительской папки

То есть, если пользователь откроет file3, то будет проверка на доступ к subfolder_3 (который readonly)

#### Сценарий №3

Если пользователь №1 изменит доступ пользователю №2 (например с readonly на writable), то рекурсивно поменяется поле permissions у монтированной папки folder (*) и у всех ее подпапок

Если пользователь №1 отключит доступ пользователю №2 к папке folder, то из бд рекурсивно удалится запись folder (*) и все ее содержимое
P.S. Поле origin_id хранит id оригинальной папки folder пользователя №1 и нужно для определения монтированной папки пользователя №2

### Выбор СУБД

#### Хранение данных

Для хранения таблиц будем использовать графовую СУБД Neo4j - она идеально подходит для реализации иерархической структуры файлов нашего облака
...
Для шардирования выберем таблицы file и folder, так как на них приходится основная нагрузка. 

Шардировать будем по юзерам

Возьмем размер максимального тарифа (1ТБ) в качества размера одного шардра

В шардах будем хранить разное количество пользователей, в зависимости от их тарифов (одного пользователя с максимальным тарифом, 10 пользователей с тарифом на 100Гб либо 124 польователя с тарифом 8гб)
...
1. Загрузка файла
   * Получение файла
     * Пользователь загружает файл => файл разбивается на чанки и грузится в S3

2. Анализ содержимого
   * Антивирусная программа развернута на сервере и в фоновом режиме проверяет файлы
     * Раз в определенное время на антивирус отправляется запрос, в котором передается путь до файла в s3, а также его id
     * Антивирус получает файл из s3, сканирует его и в случае подозрения на вредоносный контент помечает файл как опасный

3. Отображение предупреждений о вредоносных файлах
   * В списке файлов пользователя помеченные антивирусом файлы будут отображаться как опасные (значок + предупреждение при открытии)
