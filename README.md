...
|------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| users            | Таблица users хранит данные о зарегистрированных пользователях.                                                                                                                                                                                                                                                                                                                                                              |
| files            | Таблица files хранит данные о файлах. Поле owner содержит FK на uuid владельца файла. Файл может быть загружен как в корень облака, так и в папку, поэтому в поле parent хранится FK Nullable uuid родительской папки. Файлы могут быть пошерены, за это отвечает поле shared. Кроме этого файл содержит название (name), дату создания (created), а также ссылку на S3 хранилище (s3_url)                                   |
| folders          | Таблица folders хранит данные о папках. Поле owner содержит FK на uuid владельца папки. Папка может быть создана как в корне облака, так и в другой папке, поэтому в поле parent хранится FK Nullable uuid родительской папки. Файлы могут быть пошерены, за это отвечает поле shared. Кроме этого файл содержит название (name), дату создания (created), а также публичную ссылку (share_url) в случае если папка пошерена |
| acl              | Таблица acl хранит информацию о пошаренных файлах / папках. Поле owner содержит FK на uuid владельца файла / папки. Nullable поля file и folder содержат FK на пошеренный файл / папку. В поле permissions определяются поля доступа к пошеренному файлу/папке (доступ только на чтение / на редактирование)                                                                                                                 |
...
|------------------|------------------|-----------------------------------------|---------------|
| users            | 50М              | 16 + 40 + 40 + 40 + 256 = 392 б         | 18,25 Гб      |
| files            | 8 400 000 000    | 16 + 16 + 40 + 4 + 1 + 128 = 205 б      | 1,57 Тб       |
| folders          | 840 000 000      | 16 + 16 + 40 + 4 + 16 + 1 + 128 = 205 б | 160,4 Гб      |
| acl              | 84 000 000       | 16 + 16 + 16 + 16 + 1 + 1 = 66 б        | 5,16 Гб       |
