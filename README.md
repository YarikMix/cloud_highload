...
Файлы пользователей будем хранить в S3 хранилище. S3 бакеты разделим по uuid пользователя.

### Шардирование

Для шардирования выберем таблицу file, так как на нее приходится основная нагрузка

В качестве ключа для шардинга используются поле created. Шарды распределяются по месяцам

### Репликация

Будем использовать схему репликации Master-Slave с синхронным Slave'ами. 

* Для таблиц **shared, user, public** применим схему с двумя синхронным Slave'ами, 
  * Будем считать что запрос на запись прошел успешно после ответа от 1 Slave'а.

* Для таблиц **file, folder** применим схему с тремя синхронным Slave'ами, 
  * Будем считать что запрос на запись прошел успешно после ответа от 2 Slave'ов.
